...
router.post('/', async (req, res, next) => {
    try {
        ...
        await pool.query(
            'INSERT INTO beta (arduinoMAC, A0, A1, A2, A3, A4, A5, A6, A7) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [arduinoMAC, A0, A1, A2, A3, A4, A5, A6, A7]
        ).catch((err) => {
            if (err.code === 'ER_DUP_ENTRY') {
                return pool.query(
                    'UPDATE beta SET A0 = ?, A1 = ?, A2 = ?, A3 = ?, A4 = ?, A5 = ?, A6 = ?, A7 = ? WHERE arduinoMAC = ?',
...
router.get('/', (req, res, next) => {
    pool.query('SELECT * FROM beta').then(([results]) => {
        res.json(results);
    }).catch((err) => {
        next(err);
